...
FRONTEND_HOST = os.environ.get('MAIN_ACCESS_URL')                       # Domain Name of your Frontend part of this application (Example: "ask.lvkasz.us" or "ask.lvkasz.us:3100")
ADMIN_HOST    = os.environ.get('ADMIN_ACCESS_URL')                      # Domain Name of your Admin Panel part of this application (Example: "admin-ask.lvkasz.us" or "admin-ask.lvkasz.us:3200")

SENDMSG_LIMIT = os.environ.get('SENDMSG_LIMIT', '10 per hour')          # Send Question Rate-limiting: How many requests are allowed in what period of time for sending questions? - '10 per hour' is default.
ADMIN_LIMIT   = os.environ.get('ADMIN_LIMIT', '50 per hour')            # Admin Rate-limiting: How many requests are allowed in what period of time for an administrator? - '50 per hour' is default.

API_KEY       = os.environ.get('AUTH_KEY', 'changeme')                  # Your secret API Authentication Key for Authorization by using API or Admin Panel (replying to questions, deleting them etc.) Please change it from 'changeme' and set it to something that has ideally +24 characters.

SQL_HOST      = os.environ.get('MYSQL_HOST', 'localhost')               # Hostname or IP Address to your MySQL/MariaDB Server where your Q&A data will be saved (questions, answers) - 'localhost' is default.
SQL_PORT      = int(os.environ.get('MYSQL_PORT', '3306'))               # Port number of your MySQL/MariaDB Server. - 3306 is default.
SQL_USER      = os.environ.get('MYSQL_USER', 'asklvkaszus')             # Username in your MySQL/MariaDB Server for this application. If you didn't created a new user yet, please do this.
SQL_PASSWORD  = os.environ.get('MYSQL_PASSWORD', 'admin')               # Password of your MySQL/MariaDB User that you provided above. Please change it from 'admin' and set it to something that has ideally +16 characters.
SQL_DATABASE  = os.environ.get('MYSQL_DATABASE', 'admin')               # Database name that will be used by your selected MySQL/MariaDB User. - If you didn't created a new database yet, please do this.

# !DEPRECATED! : redisurl = "redis://localhost:6379"
# FROM NOW, PLEASE USE THIS VARIABLES BELOW!
REDIS_HOST    = os.environ.get('REDIS_HOST', 'localhost')               # Hostname or IP Address to your Redis Server where rate-limiting and temporary data will be stored. 'localhost' is default.
REDIS_PORT    = int(os.environ.get('REDIS_PORT', '6379'))               # Port number of your Redis Server - 6379 is default.
REDIS_DBS_0   = int(os.environ.get('REDIS_DBS_0', '0'))                 # Database numbers in your Redis Server. Number '0' is for rate-limiting data.
REDIS_DBS_1   = int(os.environ.get('REDIS_DBS_1', '1'))                 # Database numbers in your Redis Server. Number '1' is for temporary data (only for storing generated ID's temporarily for now)
...
# Configuring CORS Headers to make API a little bit more secure with empty variable detection
if not FRONTEND_HOST or not ADMIN_HOST:
    CORS(app, origins = "*")
else:
    CORS(app, origins = ["https://" + host for host in [FRONTEND_HOST, ADMIN_HOST]])

# Configuring Redis Database URL and Redis Database IDs
redis_limiter_db = "redis://" + REDIS_HOST + ":" + str(REDIS_PORT) + "/" + str(REDIS_DBS_0)
redis_idgen_db = redis.StrictRedis(host=REDIS_HOST, port=REDIS_PORT, db=REDIS_DBS_1)
...
# Kolor ANSI dla cyjanu
CYAN = "\033[36m"

# Znak końca kolorowania
RESET = "\033[0m"
